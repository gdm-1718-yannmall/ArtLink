...
class UserController {
    ...
    index = async (req, res, next) => {
        try {
            ...
            if (limit && skip) {
                ...
                artPieces = await User.paginate({}, options);
            } else {
                artPieces = await User.find().populate('category').sort({ created_at: -1 }).exec();
            }
    ...
    store = async (req, res, next) => {
        try {
            const artPieceCreate = new User({
                title: req.body.title,
                synopsis: req.body.synopsis,
                author: req.body.author,
                categoryId: req.body.categoryId,
            });
            const user = await artPieceCreate.save();
    ...
    update = async (req, res, next) => {
        ...
        try {
            ...
            const user = await User.findOneAndUpdate({ _id: id }, artPieceUpdate, { new: true }).exec();
